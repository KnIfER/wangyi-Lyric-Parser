 ...
 * @date 2017/11/18
 * @modifyer KnIfER
 * updatable:可以在comparable的基础上更新加入其他自定义接口函数，以更加灵活地使用泛型。
 * @date 2018/3/31
 */
...
public class RBTree<T extends Comparable<? super T>> {
    ...
    public RBTNode<T> xxing_samsara(T val){
     ...
     ///情况二///cur///情况一/
     protected RBTNode<T> downwardNeighbour(RBTNode<T> du, T val) {
        int cmp;
        RBTNode<T> x = du;
    ...
    //上行wrap :find node minimal x,so that x.key>=val and no node with key smaller that x.key satisfies this condition.
    ...
    private RBTNode<T> upwardNeighbour_skipego(RBTNode<T> du,T val) {
        int cmp;
        RBTNode<T> x = du;
        RBTNode<T> tmpnode = null;
     ...
     */
    protected RBTNode<T> minimum(RBTNode<T> tree) {
        if (tree == null)
            return null;
     ...
     */
    protected RBTNode<T> maximum(RBTNode<T> tree) {
        if (tree == null)
            return null;
     ...
     */
    private synchronized void insert(RBTNode<T> node) {
        int cmp;
        RBTNode<T> y = null;
